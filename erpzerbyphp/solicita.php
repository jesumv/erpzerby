<?php
//version 1.1 agosto 03, 2013.
//esta version incluye celdas para el cálculo de entregas parciales.
...
include_once"php/auxsolicita.php";


$confir = true;
//variable global que permite saber si se imprimen todas las ordenes o solo las seleccionadas.
//se inicializa para imprimir todas
$_SESSION['bandimp']= 0;
...
//indica al sistema que se desea imprimir las ordenes solicitadas
    $_SESSION['bandimp']= 1;
...
//articulos a entregar 
      ...
      WHERE t1.status <10 AND t1.no_formato_tienda".$signo."7507003100025 ORDER BY t2.upc")
...
//ordenes a entregar 
      ...
      $ordenes = mysql_query("SELECT t1.orden, t1.monto_total,t1.formato_tienda,t1.no_tienda, t2.nomcorto, t1.fecha_canc, t1.obs FROM orden_resumen as t1 LEFT JOIN 
       cdi as t2 ON t1.no_tienda = t2.no_tienda WHERE  t1.status <10 AND t1.no_formato_tienda".$signo."7507003100025 ORDER BY t2.nomcorto, 
...
// seleccion de clientes con ordenes x entregar fila 1 encabezado
       $clientes= mysql_query("SELECT t1.nomcorto FROM clientes as t1 INNER JOIN orden_resumen as t2 
        on t1.id_clientes = t2.cliente_zerby INNER JOIN cdi as t3 ON t2.no_tienda = t3.no_tienda WHERE t2.status <10 
...
// variable para tomar el numero de orden que definira la columna
    ...
    for ($j =0; $j <$reng; $j++){
        ...
        //la consulta de los articulos entregados por cada orden
          $cantent= mysql_query("SELECT SUM(cajas) FROM inventario WHERE upc = $upc AND cadena = $cad AND ref = $ordact")
...
//asignación de valor 0 a los inventarios si la consulta es nula
...
if(isset($_POST['enviof'])){
        llenabases($colord,$ords,$reng,$tabla,$formato);
...
<body id="solicita">
  ...
  <!--LISTON DE ENCABEZADO ---------------------------------------------------------------------------------------->  
    <?php 
  $titulo = "SOLICITUD ALMACEN ".$formato;
...
<!--tabla 1-->
    <table id="resumen" border= "1">
        <th colspan = '2'>RESUMEN DE LA SOLICITUD</th>
            <tr>
 <?php
...
<!---campo oculto para enviar el nombre del formato de las ordenes al reporte impreso--->
       <?php
             ...
             echo "<td align='center' colspan = $colord><input type='submit' name ='enviof' value='SOLICITADO' /></td>"; 
       ?> 
...
//fila de clientes
        ...
        <tr>
            <td class="in">ESTADO</td>
                    <?php
//fila de ESTADO DE LA ORDEN
             for($i=0; $i<$colord;$i++){
                $formato = colorestado($ords[$i]);
                echo "<td class= '$formato[1]' align='center' colspan='2'>$formato[0]</td>";  
             }
        ?> 
        </tr>
    ...
    // el numero del renglon                
                   ...
                   //upc y descripcion   
                    ...
                    //celda oculta con el peso
                     echo "<td id='pes$j' style= 'display:none' >".$tabla[$j][3]."</td>"; 
                    //celda oculta con el volumen
                    echo "<td id='vol$j' style= 'display:none' >".$tabla[$j][4]."</td>"; 
                    //celda oculta con el precio
                    echo "<td id='prec$j' style= 'display:none' >".$tabla[$j][5]."</td>"; 
                    //celda oculta con empaque
                     echo "<td id='emp$j' style= 'display:none' >".$tabla[$j][6]."</td>"; 
