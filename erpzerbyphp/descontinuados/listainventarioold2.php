        ...
        foreach($row as $cell){
          ...
          $col1++;
           //consulta para la seleccion del producto en almacen CURTIDORES
                    $query = "SELECT SUM(cajas) FROM inventario WHERE upc = $lupc[$reng] and almacen = 1";
                    $curt=mysql_query($query)or die ("Error al seleccionar inventarios zerby.".mysql_error());
                    $curt2=mysql_fetch_row($curt);
                    $curt3=$curt2[0];
                    echo "<td name = 'curt'.$reng>$curt3</td>";
          
          //consulta para la seleccion del producto en almacen zerby
                    $query = "SELECT SUM(cajas) FROM inventario WHERE upc = $lupc[$reng] and almacen = 2";
                    $zer=mysql_query($query)or die ("Error al seleccionar inventarios zerby.".mysql_error());
