...
if(isset($_POST['enviof'])){
 ...
 // 1.-se eligio la opcion todos por lo que no hay clausula WHERE en la consulta.
        ...
        if ($cte == '0'){
            $resultf = mysql_query("SELECT t1.upc,t2.nombre, t1.desc1,t1.cadena  FROM cat_arts AS t1
...
<!-- construccion de combo de filtrado ---------------------------------------------------------------->    
 ...
 <!--el combo de cliente -->
    ...
    <tr>";
        ...
        echo "<td colspan = '2'align='Center'>EXISTENCIA CAJAS</td>";
        echo "</tr>\n";
    ...
    //prints number of row
        ...
        foreach($row as $cell){
          ...
          $col1++;
           //consulta para la seleccion del producto en almacen CURTIDORES
                    $query = "SELECT SUM(cajas) FROM inventario WHERE upc = $lupc[$reng] AND cadena = '$lcadn[$reng]' AND almacen = 1";
          ...
          //consulta para la seleccion del producto en almacen zerby
                    $query3 = "SELECT SUM(cajas) FROM inventario WHERE upc = $lupc[$reng] AND cadena = '$lcadn[$reng]' AND almacen = 1";
                    $zer=mysql_query($query3)or die ("Error al seleccionar inventarios zerby.".mysql_error());
                    $zer2=mysql_fetch_row($zer);
                    $zer3=$zer2[0];
                    echo "<td name = 'zer'.$reng>$zer3</td>";
