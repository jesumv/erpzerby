...
$query2 = "SELECT num_confir, no_tienda FROM orden_resumen WHERE status = 9 || status = 10" ;
...
//ciclo de citas--------------------------------------------------------------------
       while ($citas2 = mysql_fetch_row($citas1)) {
           ...
           $datosord = mysql_query("SELECT orden,formato_tienda,obs,confirma,fecha_confir, hora_confir
            FROM orden_resumen WHERE num_confir = $citaact")or die ("Error en la consulta de citas2.".mysql_error());
           ...
           WHERE t2.num_confir = $citaact ORDER BY t1.upc ")
